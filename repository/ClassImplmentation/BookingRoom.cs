    ...
    public class BookingRoom : IBook
    {
        ...
        public void Delete_Book(int id)
        {
            ...
            if (booking != null)
            {
                if (hotel != null)
                {
                    if (booking.Room_type == "Single" && hotel.SingleRoom_Number <= 10)
                    {
                        hotel.SingleRoom_Number += 1;
                        DbContext.Update(hotel);

                    }
                    else if (booking.Room_type == "Double" && hotel.DoubleRoom_Number <= 10)
                    {
                        hotel.DoubleRoom_Number += 1;
                        DbContext.Update(hotel);
                    }
                    else if (booking.Room_type == "Triple" && hotel.TripleRoom_Number <= 4)
                    {
                        hotel.TripleRoom_Number += 1;
                        DbContext.Update(hotel);
                    }
